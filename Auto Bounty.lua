...
repeat wait() until game:GetService("ReplicatedStorage"):FindFirstChild("Effect") and game:GetService("ReplicatedStorage").Effect:FindFirstChild("Container")
...
getgenv().Config = {
    ...
    ["Chat"] = {
        ["Enabled"] = false,
        ["Message"] = {"No skill? use RIPPER Hub", "by Alphix & God N", "Get Good Get RIPPER Hub", ".gg/p4nbCfs7zW"}
    },
    ["Custom Y Run"] = {
        ["Enabled"] = false,
        ["Y Run"] = 100
    ...
    if game.Players.LocalPlayer.PlayerGui["Main (minimal)"]:FindFirstChild("ChooseTeam") then
        repeat wait()
            if game.Players.LocalPlayer.PlayerGui:FindFirstChild("Main (minimal)").ChooseTeam.Visible then
                if getgenv().Config["Team"] == "Pirate" then
                    for i, v in pairs(getconnections(game.Players.LocalPlayer.PlayerGui["Main (minimal)"].ChooseTeam.Container.Pirates.Frame.TextButton.Activated)) do                                                                                                
                        v.Function()
                    end
                elseif getgenv().Config["Team"] == "Marine" then
                    for i, v in pairs(getconnections(game.Players.LocalPlayer.PlayerGui["Main (minimal)"].ChooseTeam.Container.Marines.Frame.TextButton.Activated)) do                                                                                                
                        v.Function()
                    end
                else
                    for i, v in pairs(getconnections(game.Players.LocalPlayer.PlayerGui["Main (minimal)"].ChooseTeam.Container.Pirates.Frame.TextButton.Activated)) do                                                                                                
...
MainFrame.Size = UDim2.new(0, 413, 0, 61)
...
ScriptName.BackgroundColor3 = Color3.fromRGB(150, 255, 150)
...
ScriptName.Text = "RIPPER Hub Auto Bounty | Next Generation"
...
Logo.Image = "rbxassetid://122504813117904"
...
ClickSkipTarget.Text = "Skip Player"
...
local foldername = "RIPPER HUB"
local filename = game.Players.LocalPlayer.Name.."_AutoBounty.json"
...
function checkcombat()
    return game:GetService("Players").LocalPlayer.PlayerGui.Main.BottomHUDList.InCombat.Visible and game:GetService("Players").LocalPlayer.PlayerGui.Main.BottomHUDList.InCombat.Text and (string.find(string.lower(game:GetService("Players").LocalPlayer.PlayerGui.Main.BottomHUDList.InCombat.Text),"risk"))
end 
function GetItem(vu)
...
function to(Pos)
    ...
    if not game.Players.LocalPlayer.Character.HumanoidRootPart:FindFirstChild("Pear Cat ") then
    ...
    if Distance > distbyp and Distance > calcpos(Portal, Pos) and Distance > calcpos(Portal, Pos) and GetItem("Valkyrie Helmet") then
        return request(Portal) 
    end
    if Spawn then 
        Pos0 = calcpos(Spawn, Pos)
        if Distance > distbyp and not checkcombat() and Pos0 < Distance and calcpos(MyCFrame, Spawn) > Pos0 and _G.Number < 2 then
            return bypass(Spawn)
        end
    end
